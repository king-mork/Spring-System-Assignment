...
void MakeScene(void)
{
	...
	*/
	...
	success = GlobalResourceManager::use()->addSystem(ground, true);
	assert(success);

	// Register particles object
	Particles* partSys = new Particles("partSys");
	success = GlobalResourceManager::use()->addSystem( partSys, true );
	assert( success );

	// Register particle simulator
	SpringSim* partSim = new SpringSim("partSim", ground);
